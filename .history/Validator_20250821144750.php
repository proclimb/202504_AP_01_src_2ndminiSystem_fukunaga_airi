...
class Validator
{
    ...
    public function validate($data)
    {
        ...
        if (empty($trimmed_name)) {
            $this->error_message2['name'] = 'スペースのみでは入力できません ';
        } elseif (mb_strlen($trimmed_name) > 20) {
            $this->error_message2['name'] = '名前は20文字以内で入力してください';
        } elseif (!preg_match('/^[ぁ-んァ-ヶー一-龠々ｦ-ﾟー\s　]+$/u', $trimmed_name)) {
            $this->error_message2['name'] = '名前に使用できない文字が含まれています';
        ...
        if (empty($trimmed_kana)) {
            $this->error_message2['kana'] = 'スペースのみでは入力できません ';
        } elseif (preg_match('/[^ぁ-んー]/u', $trimmed_kana)) {
            $this->error_message2['kana'] = 'ひらがなで入力してください';
        } elseif (mb_strlen($trimmed_kana) > 20) {
            $this->error_message2['kana'] = 'ふりがなは20文字以内で入力してください';
        }


        // 生年月日
        if (empty($data['birth_year']) || empty($data['birth_month']) || empty($data['birth_day'])) {
            $this->error_message2['birth_date'] = '生年月日が入力されていません';
        } elseif (!$this->isValidDate($data['birth_year'] ?? '', $data['birth_month'] ?? '', $data['birth_day'] ?? '')) {
            $this->error_message2['birth_date'] = '生年月日が正しくありません';
        } elseif (strtotime($data['birth_year'] . '-' . $data['birth_month'] . '-' . $data['birth_day']) > strtotime(date('Y-m-d'))) {
            $this->error_message2['birth_date'] = '生年月日が未来日になっています。正しい日付を入力してください ';
        }

        // 郵便番号
        if (empty($data['postal_code'])) {
            $this->error_message2['postal_code'] = '郵便番号が入力されていません';
        } elseif (!preg_match('/^\d{3}-?\d{4}$/', $data['postal_code'] ?? '')) {
            $this->error_message2['postal_code'] = '郵便番号はXXX-XXXX または XXXXXXX の形式で入力してください';
        }
        ...
        if (empty($trimmed_pref) || empty($trimmed_city)) {
            $this->error_message2['address'] = '入力値の前後に空白（スペース）は含めないでください ';
        } elseif (mb_strlen($trimmed_pref) > 10) {
            $this->error_message2['address'] = '都道府県は10文字以内で入力してください';
        } elseif (mb_strlen($trimmed_city) > 50 || mb_strlen($trimmed_building) > 50) {
            $this->error_message2['address'] = '市区町村・番地もしくは建物名は50文字以内で入力してください';
        }

        // 電話番号
        if (empty($data['tel'])) {
            $this->error_message2['tel'] = '電話番号が入力されていません';
        } elseif (
            !preg_match('/^0\d{1,4}-\d{1,4}-\d{3,4}$/', $data['tel'] ?? '') ||
            mb_strlen($data['tel']) < 12 ||
            mb_strlen($data['tel']) > 13
        ) {
            $this->error_message2['tel'] = '電話番号は12~13桁で正しく入力してください';
        }
        // メールアドレス
        if (empty($data['email'])) {
            $this->error_message2['email'] = 'メールアドレスが入力されていません';
        } elseif (!filter_var($data['email'], FILTER_VALIDATE_EMAIL)) {
            $this->error_message2['email'] = '有効なメールアドレスを入力してください';
        }
        ...
        if (!empty($data['postal_code']) && !empty($data['prefecture']) && !empty($data['city_town'])) {
            try {
                ...
                if ($count == 0) {
                    $this->error_message2['address'] = '郵便番号と住所が一致しません';
                }
            } catch (\PDOException $e) {
                $this->error_message2['address'] = 'DBエラー: ' . $e->getMessage();
            }
        }

        return empty($this->error_message2);
    }
