 ...
 * 更新・削除画面（edit.php）
 */
require_once 'Db.php';       // ここで $pdo が変数として用意される
...
// フォーム値（初回＝DB値、POST時＝入力で上書き）
$form = $userData;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $form = array_merge($userData, $_POST);

    // Validator互換：birth_date 分解
    if (!empty($form['birth_date']) && preg_match('/^(\d{4})-(\d{2})-(\d{2})$/', $form['birth_date'], $m)) {
        $form['birth_year']  = $m[1];
    ...
    if ($validator->validate($form)) {
        ...
        if (!isset($form['gender'])) $form['gender'] = '1';
    }
}
...
<body class="edit-page">
    ...
    <h1 class="page-title"><a href="index.php">mini System</a></h1>
    ...
    <form
        id="edit-form"
        action="edit.php<?= $isAdmin && isset($_GET['id']) ? '?id=' . (int)$_GET['id'] : '' ?>"
        method="post"
        ...
        <div class="form-block">
            ...
            <?php if (isset($error_message['email'])): ?>
                <div class="error-msg"><?= htmlspecialchars($error_message['email'], ENT_QUOTES) ?></div>
            <?php endif; ?>
        </div>
